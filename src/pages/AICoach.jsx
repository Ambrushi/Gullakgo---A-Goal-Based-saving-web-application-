...
export default function AICoach() {
  ...
  const fetchGeminiAIResponse = async (userQuery) => {
    ...
    if (!apiKey) {
      return generateFallbackAIResponse(userQuery);
    }
...
Active Goals Summary: ${activeGoalsStr || 'No active goals yet'}.
Recent Monthly Spend: ₹${totalSpent}.

CRITICAL INSTRUCTION FOR RELEVANCE:
1. Answer the USER'S SPECIFIC QUESTION directly and accurately. Do NOT bring up or force specific active goal titles (like Avengers Tower, PS5, etc.) UNLESS the user explicitly asks about their goal or asks for goal progress!
2. If the user asks for a daily budget, 10-day plan, or pocket money advice (e.g. ₹1000 for 10 days in Pune), answer their budget question directly with clear calculations (e.g. ₹100/day = ₹50 spend + ₹50 save) without forcing goal names into the response.
3. Keep advice concise, actionable, teen-friendly, using emojis and markdown.`;
      ...
      // Gemini Models array with cost-effective Flash models prioritized
      ...
      if (isRateLimited) {
        console.warn('⚡ Gemini API Rate Limit / Quota Exceeded (429)');
      } else {
        console.warn(`⚠️ Gemini API Notice: ${lastErrorMsg}`);
      }
  ...
  const generateFallbackAIResponse = (userQuery) => {
    ...
    if (query.includes('10 days') || query.includes('10 day') || (query.includes('1000') && query.includes('plan')) || query.includes('pune')) {
      return `🗓️ **10-Day Daily Budget & Savings Plan (₹1,000 Budget in Pune):**\n\nWith **₹1,000** for **10 days**, your total daily allocation is **₹100 / day**.\n\n### 📊 Balanced 50/50 Daily Plan:\n• **Daily Spending Budget:** **₹50 / day** (₹500 total over 10 days for food, tea, or local travel)\n• **Daily Savings Target:** **₹50 / day** (₹500 total saved in your Gullak over 10 days)\n\n### 🥪 Smart Pune Budget Hacks:\n1. **Local Snacks:** Enjoy Pune Vada Pav or Cutting Chai (₹15-₹25) to easily stay under your ₹50 daily spend.\n2. **Travel:** Take PMPML city buses or walk short distances instead of autos.\n3. **Save First:** Set aside ₹50 into your savings every morning before spending! 🚀`;
    }
    ...
    if (query.includes('passive') || query.includes('income') || query.includes('earn') || query.includes('side hustle') || query.includes('make money')) {
      return `💸 **Smart Passive & Active Income Ideas for Teens & Students:**\n\n1. **Digital Freelancing & Micro-Skills:** Offer Canva graphic design, video editing, or social media setup for local businesses.\n2. **High-Yield Teen Savings & RD:** Deposit your saved ₹${user.totalSaved.toLocaleString('en-IN')} into a digital Recurring Deposit (RD) or liquid fund to earn 6-7% annual compound interest!\n3. **Sell Digital Templates:** Create budget spreadsheets, Notion templates, or study notes for classmates.\n4. **Parent Matching Challenge:** Negotiate a 1:1 match with your parents for every ₹100 you save into your Gullak goals! 🚀`;
    }
  ...
  return (
    <div className="container py-4" style={{ maxWidth: '850px' }}>
          ...
          <div className="d-flex align-items-center gap-3">
            ...
            <div>
              ...
              <span className="small text-secondary">
                Renew or upgrade now so you don't lose your AI Coach prompts!
              </span>
            </div>
          </div>
          <button 
      ...
      {usageInfo.remaining === 0 && (
        <div className="alert alert-warning border-0 shadow-sm rounded-4 d-flex align-items-center justify-content-between p-3 mb-4">
          <div className="d-flex align-items-center gap-2">
            ...
            <div>
              <strong className="d-block">Prompt Limit Reached ({usageInfo.limit}/{usageInfo.limit} for {usageInfo.plan.name})</strong>
      ...
      <motion.div 
        ...
        className="stash-card d-flex flex-column" 
        style={{ minHeight: '500px', height: 'calc(100vh - 260px)', maxHeight: '620px' }}
      >
        {/* Messages Body */}
        <div className="flex-grow-1 p-3 p-md-4 overflow-y-auto custom-chat-scroll d-flex flex-column gap-3" style={{ minHeight: 0 }}>
          <AnimatePresence>
            {messages.map(msg => (
              <motion.div
                key={msg.id}
        ...
        {/* Suggested Quick Questions with Horizontal Scroll Controls */}
        <div className="px-3 py-2 bg-light border-top d-flex align-items-center gap-2 flex-shrink-0">
          <button 
            type="button"
            className="btn btn-sm btn-light rounded-circle p-0 text-purple shadow-sm flex-shrink-0 d-flex align-items-center justify-content-center"
            style={{ width: '28px', height: '28px', fontSize: '0.85rem' }}
            onClick={() => {
              const el = document.getElementById('suggestionsPillBar');
          ...
          <div id="suggestionsPillBar" className="d-flex gap-2 overflow-x-auto custom-horizontal-scroll pb-1 flex-grow-1 align-items-center">
