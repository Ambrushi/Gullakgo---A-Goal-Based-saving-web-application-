...
export default function GoalCreation() {
  ...
  const { addGoal, calculateDailySavingRate } = useApp();
  ...
  const [dailySavingRate, setDailySavingRate] = useState('');
  const [isCustomDailyRate, setIsCustomDailyRate] = useState(false);
  const [lockIn, setLockIn] = useState(false);

  // Recalculate default daily rate when target amount or date changes unless manually customized
  useEffect(() => {
    if (targetAmount && targetDate && !isCustomDailyRate) {
      const rate = calculateDailySavingRate(targetAmount, 0, targetDate);
      setDailySavingRate(rate ? rate.toString() : '');
    }
  }, [targetAmount, targetDate, isCustomDailyRate, calculateDailySavingRate]);
  ...
  const handleSubmit = (e) => {
    ...
    const rate = dailySavingRate ? parseFloat(dailySavingRate) : calculateDailySavingRate(targetAmount, 0, targetDate);
  ...
  const autoCalculatedRate = (targetAmount && targetDate) ? calculateDailySavingRate(targetAmount, 0, targetDate) : 0;
  ...
  return (
    <div className="container py-4" style={{ maxWidth: '640px' }}>
      ...
      <div className="stash-card p-4 p-md-5 overflow-hidden">
        <AnimatePresence mode="wait">
          ...
          {step === 2 && (
            ...
            >
              <span className="badge bg-purple-subtle px-3 py-1 rounded-pill fw-bold mb-2">
                Step 2: Amount & Daily Saving Plan
              </span>
              <h4 className="brand-font mb-3 text-dark">Set Target & Daily Rate 💰</h4>
              ...
              {/* Dynamic Daily Saving Price Recommendation Card */}
              {targetAmount && targetDate && (
                <motion.div
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="p-3 rounded-4 border bg-purple-subtle mb-4"
                >
                  <div className="d-flex align-items-center justify-content-between mb-2">
                    <span className="fw-bold text-dark d-flex align-items-center gap-1">
                      <i className="bi bi-lightning-charge-fill text-warning"></i> Customize Daily Saving Price
                    </span>
                    <span className="badge bg-purple text-white">₹{dailySavingRate || autoCalculatedRate} / day</span>
                  </div>
                  <p className="small text-secondary mb-3">
                    {isCustomDailyRate
                      ? 'You have set a custom daily saving price.'
                      : `To reach ₹${parseFloat(targetAmount).toLocaleString('en-IN')} by ${targetDate}, you need to save ₹${autoCalculatedRate}/day.`}
                  </p>
                  ...
                  <div className="input-group mb-2">
                    ...
                    <input
                      type="number"
                      min="1"
                      className="form-control rounded-end-3 fs-6 border-start-0"
                      placeholder={`Calculated: ${autoCalculatedRate}`}
                      value={dailySavingRate}
                      onChange={e => {
                        setIsCustomDailyRate(true);
                        setDailySavingRate(e.target.value);
                      }}
                    />
                    <button
                      type="button"
                      className="btn btn-outline-secondary btn-sm ms-2 rounded-3"
                      onClick={() => {
                        setIsCustomDailyRate(false);
                        setDailySavingRate(autoCalculatedRate.toString());
                      }}
                    >
                      Reset Auto
                    </button>
                  ...
                  <div className="d-flex gap-2 flex-wrap">
                    {[50, 100, 200, 500].map(preset => (
                      <button
                        key={preset}
                        type="button"
                        className={`btn btn-xs rounded-pill px-3 ${dailySavingRate == preset ? 'btn-purple text-white' : 'btn-outline-purple'}`}
                        style={{ fontSize: '0.8rem' }}
                        onClick={() => {
                          setIsCustomDailyRate(true);
                          setDailySavingRate(preset.toString());
                        }}
                      >
                        ₹{preset}/day
                      </button>
